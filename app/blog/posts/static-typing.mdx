...
You may be aware that binary search is an `O(logn)` algorithm and linear search is an `O(n)` algorithm.
However, there are cases when for small values of `n`, linear search is faster than binary search.
