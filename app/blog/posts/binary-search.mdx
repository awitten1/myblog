...
Now is the point in the blog when I admit to you that it's possible to implement a branch-free binary search.
...
As a result, this code results in no branches at all.  We see below, that this version is the fastest of all of them!
...
So the picture we have is complicated.  A branch-free binary search implementation will likely always outperform
a linear search.  But if your binary search has branches (the way `std::lower_bound` does) then
linear search may be faster than binary search for small arrays with a random access pattern on some CPUs.
The takeaway is that you need to measure it!
