...
In this post we analyze the following implementations of binary search and linear search (compiled with O3 and march=native).  For completeness,
...
a particular branch will be taken (on x86 CPUs, branch instructions are the various jump instructions).
